...
def get_basis_rjh_tswi_dataset(files_list, phase, opts, spatial_size):
    ...
    orientation = opts.get('orientation', 'LPS')
    image_keys = opts.get('image_keys', ['image'])
    rescale = opts.get('rescale', False)
    ...
    if len(image_keys) > 1:  # multi-modalites
        cropper += [
            ConcatModalityD(keys=image_keys, output_key='image', axis=0)
        ]
    ...
    if phase == 'train':
        ...
        #         scale_range=[0.1, 0.1],
        #         mode=['bilinear', ]*len(image_keys)+['nearest'],
        #         padding_mode=['reflection', ]*len(image_keys)+['zeros'],
    ...
    if rescale:
        rescaler = NormalizeIntensityD(keys=image_keys)
    else:
        rescaler = None
    ...
    dataset = BasicClassificationDataset(
        ...
        orienter=Orientationd(keys=image_keys+['mask'], axcodes=orientation),
        spacer=SpacingD(
            keys=image_keys+["mask"],
            pixdim=spacing,
            mode=[GridSampleMode.BILINEAR, ]*len(image_keys)+[GridSampleMode.NEAREST]
        ),
        rescaler=rescaler,
        resizer=None,
        cropper=cropper,
...
@CLASSIFICATION_DATASETS.register('2D', 'rjh_tswi_multimodal',
    "/homes/clwang/Data/RJH/STS_tSWI/datalist_wi_mask@1229_2131-train-mm.json")
def get_oneside_dataset(files_list, phase, opts):
    opts['image_keys'] = ['image', 'image_mag']
    opts['rescale'] = True
    cfg.set_key("IMAGE_MAG", 'image_mag')
    return get_basis_rjh_tswi_dataset(files_list, phase, opts, (32,32))
