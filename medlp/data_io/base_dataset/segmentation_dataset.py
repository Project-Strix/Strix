...
class UnsupervisedSegmentationDataset2D(BasicSegmentationDataset):
    def __init__(
        ...
        rescaler: Optional[MapTransform] =  ScaleIntensityRanged(keys=["image"], a_min=0, a_max=255, b_min=0, b_max=1, clip=True),
        resizer: Optional[MapTransform] = ResizeWithPadOrCropd(keys=["image"], spatial_size=(768,768)),
        cropper: Optional[MapTransform] = RandSpatialCropd(keys=["image"], roi_size=(384,384), random_size=False),
        additional_transforms: Optional[Sequence] = None,
...
class UnsupervisedSegmentationDataset3D(BasicSegmentationDataset):
    def __init__(
        ...
        cropper: Optional[MapTransform] = RandSpatialCropd(keys=["image"], roi_size=(256,256,128), random_size=False),
        additional_transforms: Optional[Sequence] = None,
        caster: Optional[MapTransform] = CastToTyped(keys=["image"], dtype=np.float32),
        to_tensor: Optional[MapTransform] = ToTensord(keys=["image"]),
        augment_ratio: float = 0.5,
