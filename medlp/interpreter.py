...
@click.option("--config", type=click.Path(exists=True), default="",
              help='Specify config file')
@click.option("--test-files", type=str, default="",
              help="External files (json/yaml) for testing")
@click.option('--target-layer', '-L', type=click.UNPROCESSED,
              prompt=True, callback=partial(parse_input_str, dtype=str),
              help='Target network layer for cam visualization')
@click.option('--target-class', type=int, prompt=True, default=1,
              help='GradCAM target class, should be 0 for single chn output')
@click.option('--method', type=Choice(['gradcam', 'layercam']), default='gradcam',
